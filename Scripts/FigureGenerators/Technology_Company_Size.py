...
from StatisticalTests.Column_Converter import convert_size, convert_tech_experience
...
if __name__ == "__main__":
    ...
    ax = data.plot.bar(stacked=True, figsize=(8, 6), width=0.3)
    ax.set_xlabel("Automatic Test Level")
    ax.set_ylabel("Respondents Percentage (%)")
    ax.legend(title="Professional Experience")
    ax.legend(loc='upper right', bbox_to_anchor=(0.57, 1.15),
              ncol=4, fancybox=True, shadow=True)
    plt.show()
    ax.get_figure().savefig("../Figures/Technology_Company_Size.pdf", format='pdf', dpi=5000, bbox_inches='tight')